...
pub type DynCountryRepo = Arc<dyn CountryRepo + Send + Sync>;

pub struct ExampleCountryRepo;

#[async_trait]
pub trait CountryRepo {
    /// Loop up a user by their id.
    async fn get_by_name(&self, name: &String) -> Result<String, AppError>;
}

#[async_trait]
impl CountryRepo for ExampleCountryRepo {
    async fn get_by_name(&self, name: &String) -> Result<String, AppError> {
