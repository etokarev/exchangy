...
pub type DynExchangeRepo = Arc<dyn ExchangeRepo + Send + Sync>;

pub struct ExampleExchangeRepo;

#[async_trait]
pub trait ExchangeRepo {
    /// Loop up a user by their id.
    async fn convert(&self, from: String, to: String, amount: &f32) -> Result<f32, AppError> ;
}

#[async_trait]
impl ExchangeRepo for ExampleExchangeRepo {
    async fn convert(&self, from: String, to: String, amount: &f32) -> Result<f32, AppError> {
